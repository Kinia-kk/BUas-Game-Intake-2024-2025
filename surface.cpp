...
void Surface::Centre( char* a_String, int y1, Pixel color )
{
	...
	Print( a_String, x, y1, color , 3);
}

void Surface::Print(const char* a_String, int x1, int y1, Pixel color, int width) // upgraded size adjustment - posted by alex(stained) on 3dgep discord server
{
	if (!fontInitialized)
	{
		InitCharset();
